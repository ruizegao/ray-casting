...
def get_domain_loss(x, ret, output_name, input_name, self):
    ...
    jac_hook_lower = torch.autograd.functional.jacobian(self.forward, hook_lower)
    jac_hook_upper = torch.autograd.functional.jacobian(self.forward, hook_upper)
    # jac_hook_lower = JacobianOP.apply(self.forward(hook_lower), hook_lower)
    # jac_hook_upper = JacobianOP.apply(self.forward(hook_upper), hook_upper)
    normal_lower = n_lower.squeeze(-1)
    normal_upper = n_upper.squeeze(-1)
    # jac_hook_lower = torch.ones_like(normal_lower)
    # jac_hook_upper = torch.ones_like(normal_upper)
    inner_normal_loss = - torch.nn.CosineSimilarity()(jac_hook_upper, normal_upper)
    upper_normal_loss = - torch.nn.CosineSimilarity()(jac_hook_lower, normal_lower)
    distance_loss = torch.norm(hook_lower - hook_upper, dim=1)
    domain_loss = inner_normal_loss + upper_normal_loss + distance_loss
    return domain_loss
