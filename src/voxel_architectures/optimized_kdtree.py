...
class ShortStackKDTree:
    ...
    def build_from_tensors(self, node_lower_tree: Tensor, node_upper_tree: Tensor, node_type_tree: Tensor,
                           split_dim_tree: Tensor, split_val_tree: Tensor):
        self._node_lower = node_lower_tree
        self._node_upper = node_upper_tree
        self._node_type = node_type_tree
        self._split_dim = split_dim_tree
        self._split_val = split_val_tree
        self._num_nodes = len(self._node_lower)
        self._num_leaf_nodes = self._num_nodes - int(self._num_nodes / 2)
    ...
    def _order(self, dir: float, left: int, right: int):
        if dir >= 0:
            return left, right
        else:
            return right, left

    def _tMinMax(self, o: Tensor, d: Tensor, sceneMin: Tensor, sceneMax: Tensor):
        tMin = (sceneMin - o) / d
        tMax = (sceneMax - o) / d
        return tMin, tMax

    def traverse(self, rays_v_origin: Tensor, rays_v_dir: Tensor):
        n = len(rays_v_origin)  # number of rays
        # separate ray information into origin and direction
        tMin_v, tMax_v = self._tMinMax(rays_v_origin, rays_v_dir, self._node_lower[0], self._node_upper[0])
        ...
        stack = [(root, tMin_v, tMax_v, live_v.clone())]
        ...
        while (didstack or not done_v.all()) and not (tHit_v < tMax_v).all():
            ...
            if not (len(stack) == 0):
                didstack = True
                node, tMin_v, tMax_v, live_v = stack.pop()
                pushdown = False
            else:
                didstack = False
                node = root
            ...
            while not self._isLeaf(node):
                ...
                # simple calculations to find left and right child idx
                split_depth = int(log2(node))
                p_idx = 2 ** split_depth
                p_dist = node - p_idx
                node_left = 2 ** (split_depth + 1) + p_dist
                ...
                first, second = self._order(rays_v_dir[a], node_left, node_right)
                ...
                if torch.logical_or(wantNear_v, ~live_v).all() and not wantFar_v.all():
                    ...
                    stack.append((second, top_tMin_v, tMax_v, top_live_v))
