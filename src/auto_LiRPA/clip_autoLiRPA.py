...
def clip_domains(
    ...
    # update thresholds to be a Tensor if given as a scalar
    if isinstance(thresholds, float):
        thresholds = torch.full_like(dm_lb, thresholds)
    ...
    torch_inf = torch.full_like(curr_x, torch.inf)
    x_U_candidates = torch.where(lA > 0, curr_x, torch_inf)
    x_L_candidates = torch.where(lA < 0, curr_x, -torch_inf)
