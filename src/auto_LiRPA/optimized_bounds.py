...
def _get_optimized_bounds(
    ...
    global lb_iter, ub_iter
    if bound_lower or not bound_upper:
        if lb_iter is None:
            lb_iter = 0
        else:
            lb_iter += 1
    if bound_upper or not bound_lower:
        if ub_iter is None:
            ub_iter = 0
        else:
            ub_iter += 1
    ...
    for i in range(iteration):
        ...
        if bound_lower and swap_loss:
            ...
            lbias_vect = batch_dm_lb - batch_lA.bmm(xhat_vect) + (batch_lA.abs()).bmm(eps_vect)
            lbias = lbias_vect.reshape(batches, num_spec)
            atol=1e-6
            assert torch.allclose(lbias, ret_A[self.output_name[0]][self.input_name[0]]['lbias'], atol=atol), "Computed lbias does not match lbias in dict A"
            assert torch.allclose(_concretize(xhat_vect, eps_vect, lA, lbias_vect, -1).reshape(dm_lb.shape), dm_lb, atol=atol), "Concretized dm_lb does not match returned dm_lb"
            dm_ub = _concretize(xhat_vect, eps_vect, lA, lbias_vect, +1).reshape(dm_lb.shape)
            ...
            volumes = batch_cube_intersection_with_plane(x_L_reshape, x_U_reshape, lA_reshape, lbias_reshape,
                                                      dm_lb.reshape(batches * num_spec, -1), threshold, lb_iter)
            loss = volumes.sum()
            loss *= -1  # to maximize
            ...
            print(f"{num_unverified} unverified nodes out of {batches} nodes")
            ...
            if num_spec == 1 and input_dim == 3:
                ...
                for b in range(plot_batches):
                    ...
                    # ax1.set_zlim([min(dm_lb_np[b], x_L_np[2]), max(dm_ub_np[b], x_U_np[2])])
                ...
                plt.savefig(out_path + f"frame_{i}.png")
                show_plots = False
                if show_plots:
                    plt.show()
                else:
