...
def generate_implicit_from_file(input_path, mode, shift=None, dtype=torch.float32, **kwargs):
    ...
    if mode == 'sdf':
        ...
        torch_model = mlp.func_as_torch(params, dtype)
        ...
        crown_func = mlp.func_as_torch(params, dtype)
        # data_bound = 1.
        # lower = torch.tensor((0.25, -0.6875, 0.46875), dtype=torch.float32)
        # upper = torch.tensor((0.28125, -0.6875, 0.46875))
        # upper = torch.tensor((0.28125, -0.65625, 0.5), dtype=torch.float32)
