...
def cast_rays_tree_based(func_tuple, params_tuple, roots, dirs, delta=0.001):
    ...
    for plane, next_plane in zip(all_interaction_planes, all_interaction_planes[1:]):
        rays_plane_intersection, t = find_rays_plane_intersection_with_depth(roots, dirs, (plane+next_plane)/2, primary_dimension)
...
def cast_rays_iter(funcs_tuple, params_tuple, n_substeps, curr_roots, curr_dirs, curr_t, curr_int_size, curr_inds, curr_valid, curr_count, out_t, out_hit_id, out_count, opts):
    ...
    if funcs_are_crown:
        ...
        for start_idx in range(0, total_samples, batch_size_per_iteration):
            ...
            curr_t[start_idx:end_idx], curr_int_size[start_idx:end_idx], is_hit[start_idx:end_idx], hit_id[start_idx:end_idx], num_inner_steps[start_idx:end_idx] = take_several_steps_batched(batch_curr_roots, batch_curr_dirs, batch_curr_t, batch_curr_int_size)
    else:
        ...
        for start_idx in range(0, total_samples, batch_size_per_iteration):
            ...
            curr_t[start_idx:end_idx], curr_int_size[start_idx:end_idx], is_hit[start_idx:end_idx], hit_id[
                                                                                                    start_idx:end_idx], num_inner_steps[
                                                                                                                        start_idx:end_idx] = vmap(take_several_steps)(
...
def cast_rays(funcs_tuple, params_tuple, roots, dirs, opts):
    ...
    # print(out_t.shape, out_hit_id.shape, out_count.shape, N_evals)
...
def cast_rays_frustum_iter(
    ...
    def take_step(ray_xu_yu, ray_xu_yl, ray_xl_yu, ray_xl_yl, mid_ray, expand_fac, is_single_pixel, substep_ind, in_tup):
        ...
        for func, params in zip(funcs_tuple, params_tuple):
            ...
            can_step = torch.logical_and(
                            can_step,
                            torch.logical_or(torch.eq(box_type, torch.full_like(box_type, SIGN_POSITIVE)), torch.eq(box_type, torch.full_like(box_type, SIGN_NEGATIVE)))
                        )
    ...
    for start_idx in range(0, total_samples, batch_size_per_iteration):
        end_idx = min(start_idx + batch_size_per_iteration, total_samples)
        out_tup = take_several_steps(curr_frust_range[start_idx:end_idx], curr_frust_t[start_idx:end_idx], curr_frust_int_size[start_idx:end_idx])
    ...
    terminated = torch.logical_and(torch.logical_or(torch.logical_or(is_hit, is_miss), torch.full_like(is_hit, is_count_terminate)), curr_valid)
    # Write out finished rays
    ...
    can_subd = torch.logical_or(curr_frust_range[:,2] > (curr_frust_range[:,0]+1), curr_frust_range[:,3] > (curr_frust_range[:,1]+1))
...
def frustum_needs_subdiv_to_pixel(frust_valid, frust_range):
    is_single_pixel = torch.logical_and((frust_range[:,0]+1) == frust_range[:,2], (frust_range[:,1]+1) == frust_range[:,3])
