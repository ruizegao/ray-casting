...
def main():
    ...
    node_lower_valid, node_upper_valid, lAs, lbs, uAs, ubs = construct_hybrid_unknown_tree(implicit_func, params, lower, upper)
    out_valid = {}
    out_valid['lower'] = node_lower_valid.cpu().numpy()
    out_valid['upper'] = node_upper_valid.cpu().numpy()
    out_valid['mA'] = (0.5 * lAs + 0.5 * uAs).cpu().numpy()
    out_valid['mb'] = (0.5 * lbs + 0.5 * ubs).cpu().numpy()
    out_valid['lA'] = lAs.cpu().numpy()
    out_valid['lb'] = lbs.cpu().numpy()
    out_valid['uA'] = uAs.cpu().numpy()
    out_valid['ub'] = ubs.cpu().numpy()
